...
- The app lives at the repo root (not a `bmad-room-chat-ui/` subpackage — moved there after Story 1.2's stack migration), alongside this repo's existing `_bmad`/`_bmad-output`/`.claude` tooling as sibling directories (excluded from lint/typecheck scope). Top-level module split: `src/actors`, `src/pty`, `src/parsing`, `src/persistence`, `src/ws`, `src/lib`, `src/components`, `src/state`, and `src/app` (Next.js App Router, including `src/app/api/**/route.ts` for what was `src/http/`).
